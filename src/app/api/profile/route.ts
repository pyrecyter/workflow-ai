...
async function verifyTokenAndGetUserId(req: NextRequest) {
  ...
  try {
    ...
    if (!ObjectId.isValid(userIdString)) {
      return { userId: null, response: NextResponse.json({ message: 'Invalid token payload: userId is not a valid ObjectId string' }, { status: 401 }) };
    }
...
export async function PUT(req: NextRequest) {
  try {
    ...
    if (email) {
      ...
      if (existingUser && existingUser._id.toString() !== userId.toString()) { // Compare ObjectId to string
